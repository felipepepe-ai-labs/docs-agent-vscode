...
export function validateAndParse(raw: string, contextFiles: Set<string>): DocResult {
  ...
  for (const entry of parsed) {
    const reason = validateEntry(entry, contextFiles);
    if (reason) {
...
function validateEntry(entry: unknown, contextFiles: Set<string>): string | null {
  ...
  if (!contextFiles.has(e.file as string)) return `"file" does not match any // FILE: header in context: ${e.file}`;
...
export function renderMarkdown(result: DocResult, sourceFile: string, lookupImpact?: ImpactLookup): string {
  ...
  const lines: string[] = [
    ...
    '',
    `> Generated by Docs Agent. File paths verified against context bundle; line numbers are model-reported.`,
    '',
  ];
